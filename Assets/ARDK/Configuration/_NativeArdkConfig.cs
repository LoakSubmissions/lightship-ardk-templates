...
namespace Niantic.ARDK.Configuration
{
    ...
    _ArdkGlobalConfigBase
  {
    ...
    private string _scanQualityUrl;
    ...
    {
      if (!_NAR_ARDKGlobalConfigHelper_SetScanQualityUrl(url))
      {
        ARLog._Warn("Failed to set the Context Awareness URL.");
        return false;
      }
    ...
    // Set ScanQuality URL
    [DllImport(_ARDKLibrary.libraryName)]
    private static extern bool _NAR_ARDKGlobalConfigHelper_SetScanQualityUrl(string url);
